...
const RequestforRepair = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("approximateAmount", totalCost); // Assuming totalCost holds the approximateAmount value

    // Append each selected file
    if (selectedFile) {
      formData.append("damageCarImg", selectedFile);
    }

    try {
      const response = await axios.post(
        `http://localhost:8080/hub/carRepairApproval/${carRepairId}`,
        formData,
        {
          headers: {
